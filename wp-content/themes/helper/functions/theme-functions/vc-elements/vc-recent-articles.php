<?php
/**
 * Function Visual Composer Extend (Recent Articles)
 *
 * @package WordPress
 * @subpackage Helper
 * @since Helper 1.0.0
 */
if( !function_exists('warrior_vc_recent_article_shortcode') ) {
	function warrior_vc_recent_article_shortcode( $atts ) {
	    extract( shortcode_atts( array(
	    	'type'			 				=> 'in_container',
	        'recent_article_title' 			=> '',
	        'recent_article_showposts' 		=> '',
	        'recent_article_title_limiter'	=> '',
	        'css_animation' 				=> '',
	        'el_class' 						=> '',
	        'css' 							=> ''
	    ), $atts ) );
...
?>
		<?php
			...
			$args_recent_article = array(
				'post_type' 			=> 'post',
		        'post_status' 			=> 'publish',
				'ignore_sticky_posts' 	=> 1,
		        'posts_per_page' 		=> absint( $recent_article_showposts )
			);

			$wp_query = new WP_Query();
			$wp_query->query( $args_recent_article );
		    ...
		    if ( $wp_query->have_posts() ) {
		    	while ( $wp_query->have_posts() ) {
		            ...
		            $warrior_set_content .= '<li class="'.$css_animate_class.'">';
	                    ...
	                    $warrior_set_content .= '<h3 class="post-title"><a href="'.get_the_permalink().'" title="'. get_the_title() .'">' .wp_trim_words( get_the_title(), absint($recent_article_title_limiter), '...' ). '</a></h3>';
...
add_shortcode( 'warrior_vc_recent_article', 'warrior_vc_recent_article_shortcode' );
 ...
 * @since Helper 1.0.0
 */
add_action( 'vc_before_init', 'warrior_vc_recent_article_set_param' );
if( !function_exists('warrior_vc_recent_article_set_param') ) {
	function warrior_vc_recent_article_set_param() {
		vc_map( array(
			"name" 				=> esc_html__("Recent Articles", "helper"), // add a name
			"base" 				=> "warrior_vc_recent_article", // bind with our shortcode
			"description" 		=> esc_html__( "Display recent articles.", "helper" ),
			...
			"params" => array(
			    array(
			        ...
			        "param_name" 	=> "recent_article_title"
			    ),
			    array(
					'type' 			=> 'textfield',
					'heading' 		=> esc_html__( 'Post Title Limiter', 'helper' ),
					'param_name' 	=> 'recent_article_title_limiter',
					'value' 		=> 7
				),
				array(
					'type' 			=> 'textfield',
					'heading' 		=> esc_html__( 'Posts Per Page', 'helper' ),
					'param_name' 	=> 'recent_article_showposts',
					'value' 		=> 4
				),
				array(
					'type' 			=> 'dropdown',
